...
public class ShamDriveTrain {
    ...
    protected ShamDriveTrain(
        ...
        logger.log(
                ...
                (Struct<ShamDriveTrainConfig<?, ?>>)
                        ProceduralStructGenerator.extractClassStructDynamic(config.getClass())
                                .get());
        var fixture =
                chassis.addFixture(
                        Geometry.createRectangle(
                                config.bumperLengthXMeters, config.bumperWidthYMeters),
     ...
     *
     * @return a {@link Pose2d} object yielding the current world pose of the robot in the
     *     simulation
     */
     ...
     */
    @SuppressWarnings("unchecked")
    public static <T extends ShamDriveTrain, C extends ShamDriveTrainConfig<T, C>>
            T createDriveTrain(ShamRobot<T> robot, C config) {
