...
public class Tracer {
    ...
    private static final class TracerState {
        ...
        /**
         * The stack of traces, every startTrace will add to this stack and every endTrace will
        ...
        /**
         * The start time of each trace and the gc time at the start of the trace, the key is the
         * trace name, modified every startTrace and endTrace.
         */
        ...
        /**
         * If the tracer should be disabled next cycle and every cycle after that until this flag is
         * set to false. Disabling is done this way to prevent disabling/enabling.
         */
        ...
        private TracerState(String name, boolean threadLocalConstruction) {
            if (singleThreadedMode.get() && threadLocalConstruction) {
                DriverStation.reportError(
                        "[Tracer] Tracer is in single threaded mode, cannot start traces on"
                                + " multiple threads",
                        true);
            ...
            if (m_disabled != m_disableNextCycle || m_cyclePoisoned) {
                ...
                for (var traceTime : m_traceTimes.entrySet()) {
                    DoublePublisher publisher =
                            m_rootTable.getDoubleTopic(traceTime.getKey()).publish();
    ...
    private static void endTraceInner(final TracerState state) {
        ...
        if (!state.m_disabled) {
            if (stack.isEmpty()) {
                DriverStation.reportError(
                        "[Tracer] Stack is empty,this means that there are more endTrace calls than"
                                + " startTrace calls",
                        true);
                return;
            }
            ...
            state.m_traceTimes.put(
                    stack,
                    Timer.getFPGATimestamp() * 1_000.0 - startData.m_startTime - gcTimeSinceStart);
        }
    ...
    /**
     * Starts a trace, should be called at the beginning of a function that's not being called by
     * user code.
     *
     ...
     * throw an exception. This will shorten the path of traced data in NetworkTables by not
     * including the thread name.
     *
     ...
     * This will cause all values to appear as if they're frozen at the value they were at when this
     * function was called.
     */
     ...
     *
     * @deprecated This constructor is only for compatibility with the 2024 {@code Tracer} and will
     *     be removed in 2025. Use the static methods in {@link Tracer} instead.
     */
