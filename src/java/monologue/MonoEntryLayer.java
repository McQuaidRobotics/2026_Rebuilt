                ...
                .orElseThrow(
                        () ->
                                new IllegalStateException(
                                        "Monologue wpilog not initialized or not available"));
    }
    ...
    public static interface MonologueEntry<T> {
            ...
            if (!map.containsKey(path)) {
                ...
                if (ALWAYS_LOG_BOTH) {
                    ...
                    e =
                            switch (sink) {
                                case NT ->
                                        new MonologueNtEntry<>(cleanPath, Optional.empty(), clazz);
                                case DL ->
                                        new MonologueFileEntry<>(
                                                cleanPath, Optional.empty(), clazz);
                                case OP ->
        ...
        public static <T> MonologueEntry<T> create(String path, Struct<T> struct, LogSink sink) {
            ...
            if (!map.containsKey(path)) {
                ...
                if (ALWAYS_LOG_BOTH) {
                    e =
                            new MonologueDualEntry<>(
                                    cleanPath, Optional.of(struct), struct.getTypeClass(), sink);
                } else {
                    e =
                            switch (sink) {
                                case NT ->
                                        new MonologueNtEntry<>(
                                                cleanPath,
                                                Optional.of(struct),
                                                struct.getTypeClass());
                                case DL ->
                ...
                if (ALWAYS_LOG_BOTH) {
                    ...
                    e =
                            switch (sink) {
                                case NT ->
                                        new MonologueNtEntry<>(
                                                cleanPath, Optional.of(struct), clazz);
                                case DL ->
                                        new MonologueFileEntry<>(
                                                cleanPath, Optional.of(struct), clazz);
                                case OP ->
            ...
            if (optStruct.isPresent()) {
                if (clazz.isArray()) {
                    ...
                    StructLogEntry<T> entry =
                            StructLogEntry.create(dl, path, (Struct<T>) optStruct.get());
        ...
        public MonologueNtEntry(String path, Optional<Struct<?>> optStruct, Class<T> clazz) {
            ...
            if (optStruct.isPresent()) {
                if (clazz.isArray()) {
                    ...
                    StructPublisher<T> entry =
                            nt.getStructTopic(path, (Struct<T>) optStruct.get()).publish();
                    ntLog = entry::set;
                }
            } else if (clazz.equals(Double.class) || clazz.equals(double.class)) {
