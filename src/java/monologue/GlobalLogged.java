...
class GlobalLogged {
     ...
     */
    @SuppressWarnings("unchecked")
    public static <R extends StructSerializable> R[] log(
            String entryName, R[] value, LogSink sink) {
        if (!Monologue.hasBeenSetup() || Monologue.isMonologueDisabled()) {
            String entryNameFinal = entryName;
            Monologue.prematureLog(() -> GlobalLogged.log(entryNameFinal, value, sink));
            return value;
        ...
        Struct<R> struct =
                (Struct<R>)
                        ProceduralStructGenerator.extractClassStructDynamic(
