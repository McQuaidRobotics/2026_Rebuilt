...
public class SwerveSetpointGenerator {
    ...
    public SwerveSetpoint generateSetpoint(
        ...
        for (int m = 0; m < NUM_MODULES; m++) {
            if (vars.steeringOverride[m] != null) {
                if (shouldFlipHeading(
                        retStates[m]
                                .angle
                                .unaryMinus()
                                .rotateBy(vars.steeringOverride[m])
                                .getRadians())) {
            ...
            final double deltaRotation =
                    angularDifference(
                            vars.prevModuleStates[m].angle.getRadians(),
            ...
            final double deltaVelocity =
                    retStates[m].speedMetersPerSecond
                            - vars.prevModuleStates[m].speedMetersPerSecond;
            outputStates[m] =
    ...
    private static void makeVectors(LocalVars vars) {
        ...
        vars.dtheta =
                vars.desiredSpeeds.omegaRadiansPerSecond - vars.prevSpeeds.omegaRadiansPerSecond;
    }
    ...
    private void solveSteering(LocalVars vars) {
        ...
        for (int m = 0; m < NUM_MODULES; m++) {
            ...
            if (epsilonEquals(vars.prevModuleStates[m].speedMetersPerSecond, 0.0)) {
                ...
                double requiredRots =
                        angularDifference(vars.prev[m].radians(), vars.desired[m].radians());
                if (shouldFlipHeading(requiredRots)) {
                    requiredRots =
                            rotateBy(
                                    requiredRots,
                                    Rotation2d.k180deg.getCos(),
                                    Rotation2d.k180deg.getSin());
                }
                    ...
                    Rotation2d adjusted =
                            vars.prevModuleStates[m].angle.rotateBy(
                                    Rotation2d.fromRadians(
                                            Math.signum(requiredRots) * maxThetaStep));
            ...
            double maxHeadingChange =
                    (vars.dt * wheelFrictionForce)
                            / ((massKg / NUM_MODULES)
                                    * Math.abs(vars.prevModuleStates[m].speedMetersPerSecond));
    ...
    private void solveDriving(LocalVars vars) {
        ...
        for (int m = 0; m < NUM_MODULES; m++) {
            ...
            // Translation2d moduleForceVec = new Translation2d(forceAtCarpet * forceSign,
            // forceAngle);
