...
/**
 * Centered management class for the Wayfinder library.
 * Use this to register obstacles, calculate paths, and toggle debug visualizations.
 */
public class WayfinderManager {
    ...
    /**
     * Configure the Wayfinder system with robot-specific controllers and kinematics.
     * This should be called once during robot initialization.
     */
    public static void setup(PositionalController positionalController, SwerveSetpointGenerator setpointGenerator) {
     ...
     * @param currentSpeeds Robot's current measured speeds
    ...
    public static SwerveSetpoint calculate(
        ...
        FieldSpeeds desiredFieldSpeeds = planner.calculate(dt, currentPose, currentSpeeds, targetPose, constraints);

        // 2. Constrain speeds to physical limits (Torque, Friction, Kinematics)
        lastSetpoint = generator.generateSetpoint(
                lastSetpoint, 
                currentPose.getRotation(), 
                Speeds.fromFieldRelative(desiredFieldSpeeds.vx(), desiredFieldSpeeds.vy(), desiredFieldSpeeds.omega()), 
                Optional.of(constraints), 
                dt);

        // 3. Debug visualization
        if (arrowsEnabled) {
            Logger.recordOutput("Pose/Wayfinder/Arrows", planner.getArrows(targetPose.getTranslation(), 16, 8));
        }
    ...
    private static void refreshPlanner() {
        if (controller != null) {
            planner = new RepulsorFieldPlanner(controller, registeredObstacles.toArray(new Obstacle[0]));
        }
    }
}
