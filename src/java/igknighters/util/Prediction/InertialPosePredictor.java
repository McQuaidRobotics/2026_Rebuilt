...
public class InertialPosePredictor {
    ...
    private static class InertialSample {
        ...
        public InertialSample(Pose2d pose, ChassisSpeeds speeds, double gyroOmega, double fusedAx, double fusedAy, double alpha, double timestamp) {
            this.pose = pose;
            this.speeds = speeds;
    ...
    public synchronized void update(
        ...
        if (history.isEmpty()) {
            history.addLast(new InertialSample(currentPose, robotSpeeds, gyroOmega, 0, 0, 0, timestamp));
            return;
        }
        ...
        history.addLast(new InertialSample(currentPose, robotSpeeds, gyroOmega, fusedAx, fusedAy, alpha, timestamp));
    ...
    public synchronized Pose2d predict(double lookaheadTimeSeconds) {
        ...
        InertialSample older = history.get(history.size() - 3); // Look back ~40ms for Jerk calculation
