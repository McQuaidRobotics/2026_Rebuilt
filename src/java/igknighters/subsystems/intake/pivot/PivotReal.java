...
public class PivotReal extends Pivot {
    ...
    public PivotReal() {
        pivotMotor = new TalonFX(Robot.consts.intake().kPivot().MOTOR_ID(),Robot.consts.intake().kCANBUS());
        pivotMotor.getConfigurator().apply(getPivotConfig());

        pivotEncoder =
                new CANcoder(Robot.consts.intake().kPivot().CANCODER_ID(),Robot.consts.intake().kCANBUS());
    ...
    public TalonFXConfiguration getPivotConfig() {
        ...
        config.CurrentLimits.SupplyCurrentLimit =Robot.consts.intake().kPivot().SUPPLY_UPPER_LIMIT();
