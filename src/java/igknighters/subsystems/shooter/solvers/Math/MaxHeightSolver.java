...
public class MaxHeightSolver extends Solver {
    ...
    @Override
    public ShooterState solve(Supplier<Pose3d> shooterPose, Supplier<Pose3d> targetPose,
            Supplier<ChassisSpeeds> chassisSpeeds, double currentRPM, double maxHeightMeters) {
            ...
            for (int i = 0; i < 10; i++) {
                ...
                double RPM = (v_total / (2 * Math.PI * SubsystemConstants.kShooter.kFlywheels.WHEEL_RADIUS_METERS)) * 60 * 2.0;
                ...
                if (hoodAngleDegrees < SubsystemConstants.kShooter.kHood.MIN_ANGLE_DEGREES
                        || hoodAngleDegrees > SubsystemConstants.kShooter.kHood.MAX_ANGLE_DEGREES) continue;
            ...
            double robotYawFuture =
                    shooterPose.get().getRotation().getZ() + chassisSpeeds.get().omegaRadiansPerSecond * 0.02;
            double turretAngle =
                    Math.atan2(
