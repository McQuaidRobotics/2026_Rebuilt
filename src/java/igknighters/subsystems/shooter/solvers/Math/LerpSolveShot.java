...
public class LerpSolveShot {
    ...
    static LerpTable TANGENTIAL =
	    new LerpTable(
		    new LerpTableEntry[] {
			new LerpTableEntry(1.0, 0.7),
			new LerpTableEntry(2.0, 0.8),
			new LerpTableEntry(3.0, 0.9),
			new LerpTableEntry(4.5, 1.0),
			new LerpTableEntry(5.0, 1.1)
		    });
	...
	Translation2d vectorToGoal =
		goalPose.getTranslation().toTranslation2d().minus(shooterPose.getTranslation().toTranslation2d());
	...
	double radialMultiplierToUse = (radialVelocityMag >= 0) ? radialTowardsMultiplier : radialAwayMultiplier;
	...
	for (int i = 0; i < 2; i++) {
	    ...
	    FieldVisualizer.getInstance()
		    .updateShootingTarget(
			    new Pose2d(
				    goalPose.getTranslation().toTranslation2d().minus(movingCompensation),
				    new Rotation2d()));
	    ...
	    Translation2d requiredShooterVector = fieldRelativeVelocityVector.minus(tunedRobotVelocity);
