...
public class FullIntake extends SubsystemBase {
    ...
    private SmartMotorController pivotSMC =
            new TalonFXWrapper(pivotMotor, DCMotor.getKrakenX60(1), pivotConfig);

    private ArmConfig armCfg =
            new ArmConfig(pivotSMC)
    ...
    private SmartMotorControllerConfig rollersSMCConfig =
            new SmartMotorControllerConfig(this)
                    .withControlMode(ControlMode.CLOSED_LOOP)
                    // Feedback Constants (PID Constants)
    ...
    private SmartMotorController rollerSmartMotorController =
            new TalonFXWrapper(roller, DCMotor.getKrakenX60(1), rollersSMCConfig);
    ...
    // Shooter Mechanism
    private FlyWheel rollers = new FlyWheel(shooterConfig);


    public Command targetState(YamIntakeState state) {
        return Commands.parallel(
            () -> rollers.setMechanismVelocitySetpoint(state.rollerVelocity),
            () -> arm.setMechanismPositionSetpoint(state.pivotAngle));
    }
    ...
    public boolean isAt(
            YamIntakeState state, AngularVelocity velocityTolerance, Angle angleTolerance) {
        return Math.abs(this.getRollerVelocity().in(RPM) - state.rollerVelocity.in(RPM))
                        < velocityTolerance.in(RPM)
                && Math.abs(this.getPivotAngle().in(Degrees) - state.pivotAngle.in(Degrees))
